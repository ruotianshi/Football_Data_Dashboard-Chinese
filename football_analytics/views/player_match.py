...
from utils.match_helpers import extract_match_side_info
...
def render():
    ...
    if player_id is None and not player_name:
        st.info(t("Select a player from the sidebar."))
        return

    # 加载单场球员统计
    with st.spinner(t("Loading player match data...")):
    ...
    if match_player_df.empty and events_df.empty:
        st.warning(t("No player match stats or event data available for this match."))
        return
    ...
    if player_events.empty and player_row is None:
        st.warning(t("Player not found in this match data."))
        return
...
def _render_sidebar_filters():
    with st.sidebar:
        ...
        if match_opts:
            ...
            sel_match = st.selectbox(t("Match"), match_labels,
...
def _build_match_opts(matches_df: pd.DataFrame) -> dict:
    ...
    for _, row in matches_df.sort_values("match_date").iterrows():
        ...
        hn = home_info["name"] or t("Home")
        an = away_info["name"] or t("Away")
    ...
    with col_info:
        ...
        <div style="padding: 4px 0">
            ...
            <p style="margin:3px 0; color:{COLORS['muted']}; font-size:0.85rem">
                🏟️ {team_name} &nbsp;·&nbsp; 📍 {pos} &nbsp;·&nbsp; ⏱️ {minutes} {t('Minutes')}
            </p>
        </div>
    ...
    for chunk in chunks:
        ...
        for col_ui, (label, key, is_pct) in zip(cols, chunk):
            ...
            col_ui.metric(
                ...
                help=(
                    t("Season avg: {value}", value=_format_metric_value(season_value, pct=is_pct))
...
def _render_pitch_tab_shots(player_events: pd.DataFrame, player_row, player_name: str):
    ...
    with col_stats:
        st.markdown(f"**{t('Shot Summary')}**")
        _metric_value_line(t("Shot Attempts"), attempts)
        _metric_value_line(t("Goals"), goals)
        _metric_value_line(t("Total xG"), total_xg, precision=2)
        st.caption(t("{count} shot events", count=attempts))

    with col_pitch:
        fig = _build_shots_pitch_figure(shot_events, player_name)
        st.plotly_chart(fig, use_container_width=True)
        if shot_events.empty:
            st.caption(t("No shot events found for this player in the loaded event data."))
...
def _render_pitch_tab_passes(player_events: pd.DataFrame, player_row, player_name: str):
    ...
    with col_stats:
        st.markdown(f"**{t('Pass Summary')}**")
        _metric_mini(t("Passes"), player_row, "player_match_passes")
        _metric_value_line(t("Pass Rate"), pass_rate, pct=True)
        _metric_mini(t("Forward"), player_row, "player_match_forward_passes")
        _metric_mini(t("Backward"), player_row, "player_match_backward_passes")
        _metric_mini(t("Sideways"), player_row, "player_match_sideways_passes")
        _metric_mini(t("Key Passes"), player_row, "player_match_key_passes")
        show_incomplete = st.toggle(t("Show Incomplete Passes"), value=True, key="pms_pass_incomplete")
        st.caption(t("{count} pass events", count=len(pass_events)))

    with col_pitch:
        fig = _build_passes_pitch_figure(pass_events, player_name, show_incomplete=show_incomplete)
        st.plotly_chart(fig, use_container_width=True)
        if pass_events.empty:
            st.caption(t("No pass events found for this player in the loaded event data."))
...
def _render_pitch_tab_carries(player_events: pd.DataFrame, player_row, player_name: str):
    ...
    positive_carries, negative_carries = _count_carry_obv(carry_events)

    with col_stats:
        st.markdown(f"**{t('Carry OBV Summary')}**")
        _metric_value_line(t("Positive OBV Carries"), positive_carries)
        _metric_value_line(t("Negative OBV Carries"), negative_carries)
        st.caption(t("{count} carry events in total", count=carry_attempts))

    with col_pitch:
        fig = _build_carries_pitch_figure(carry_events, player_name)
        st.plotly_chart(fig, use_container_width=True)
        if carry_events.empty:
            st.caption(t("No carry events found for this player in the loaded event data."))


def _render_pitch_tab_defensive(player_events: pd.DataFrame, player_row, player_name: str):
    col_pitch, col_stats = st.columns([7, 3], gap="medium")
    defensive_type_options = ["Ball Recovery", "Clearance", "Block", "Pressure", "Duel"]
    if any(value not in defensive_type_options for value in st.session_state.get("pms_def_types", [])):
        st.session_state["pms_def_types"] = ["Ball Recovery", "Block", "Duel"]
    defensive_types = st.multiselect(
        t("Select Defensive Action Types"),
        options=defensive_type_options,
    ...
    with col_stats:
        st.markdown(f"**{t('Defensive Summary')}**")
        all_defensive_counts = _defensive_action_counts(player_events)
        _metric_value_line(t("Ball Recovery"), all_defensive_counts.get("Ball Recovery", 0))
        _metric_value_line(t("Block"), all_defensive_counts.get("Block", 0))
        _metric_value_line(t("Pressure"), all_defensive_counts.get("Pressure", 0))
        _metric_value_line(t("Duel"), all_defensive_counts.get("Duel", 0))
        st.caption(t("{count} defensive events", count=len(defensive_events)))

    with col_pitch:
        fig = _build_defensive_pitch_figure(defensive_events, player_name)
        st.plotly_chart(fig, use_container_width=True)
        if defensive_events.empty:
            st.caption(t("No selected defensive action events found for this player in the loaded event data."))


def _render_pitch_tab_heatmap(player_events: pd.DataFrame, player_row, player_name: str):
    col_pitch, col_stats = st.columns([7, 3], gap="medium")
    location_events = _events_with_location(player_events, on_ball_only=True)

    with col_stats:
        st.markdown(f"**{t('On-Ball Activity Summary')}**")
        _metric_mini(t("Minutes"), player_row, "player_match_minutes")
        _metric_mini(t("Touches"), player_row, "player_match_touches", alt_keys=["player_match_touch"])
        _metric_value_line(t("Passes"), len(_events_of_type(location_events, "Pass")))
        _metric_value_line(t("Carries"), len(_events_of_type(location_events, "Carry")))
        _metric_value_line(t("Shots"), len(_events_of_type(location_events, "Shot")))
        st.caption(t("{count} on-ball events with location", count=len(location_events)))

    with col_pitch:
        fig = _build_heatmap_pitch_figure(location_events, player_name)
        st.plotly_chart(fig, use_container_width=True)
        if location_events.empty:
            st.caption(t("No on-ball location events found for this player, so the heatmap is empty."))
...
def _render_event_summary_cards(player_events: pd.DataFrame):
    ...
    c1.metric(t("Shots"), shots)
    c2.metric(t("Passes"), passes)
    c3.metric(t("Carries"), carries)
    c4.metric(t("Defensive Actions"), defensive)
...
def _extract_match_side_info(match_row, side: str) -> dict:
    return extract_match_side_info(match_row, side)
...
def _events_with_location(events_df: pd.DataFrame, on_ball_only: bool = False) -> pd.DataFrame:
    if events_df.empty:
        return pd.DataFrame()
    location_series = _event_location_series(events_df)
    if location_series is None:
        return pd.DataFrame()
    filtered = events_df[location_series.apply(_is_valid_location)].copy()
    if not on_ball_only or filtered.empty:
        return filtered

    type_series = _event_type_series(filtered)
    if type_series is None:
        return pd.DataFrame()
    return filtered[type_series.astype(str).isin(ON_BALL_EVENT_TYPES)].copy()
...
def _build_shots_pitch_figure(shot_events: pd.DataFrame, player_name: str) -> go.Figure:
    ...
    """
    fig = _create_half_pitch_figure(f"{player_name} · {t('Shots')}")
    if shot_events.empty:
        return fig
    shown_legend = set()
    for _, shot in shot_events.iterrows():
        ...
        hover_text = (
            ...
            f"{t('Minute')}: {shot.get('minute', 'N/A')}<br>"
            f"{t('Outcome')}: {outcome}<br>"
            f"{t('Technique')}: {_event_value(shot, ['shot_technique', 'shot_technique_name'], 'N/A')}<br>"
            f"xG: {float(_event_value(shot, ['shot_statsbomb_xg'], 0) or 0):.3f}<br>"
            f"{t('Body Part')}: {body_part}"
        )
...
def _build_passes_pitch_figure(pass_events: pd.DataFrame, player_name: str, show_incomplete: bool) -> go.Figure:
    fig = _create_pitch_figure(f"{player_name} · {t('Passes')}")
    if pass_events.empty:
        return fig
        ...
        for pass_height in ["Ground Pass", "Low Pass", "High Pass"]:
            ...
            for _, event in subset.iterrows():
                ...
                if x0 is None or x1 is None:
                    continue
                outcome_label = t("Success") if is_success else t("Incomplete")
                hover_text = (
                    f"<b>{player_name}</b><br>"
                    f"{t('Minute')}: {event.get('minute', 'N/A')}<br>"
                    f"{t('Height')}: {pass_height}<br>"
                    f"{t('Body Part')}: {_event_value(event, ['pass_body_part', 'pass_body_part_name'], 'N/A')}<br>"
                    f"{t('Outcome')}: {outcome_label}"
                )
...
def _build_carries_pitch_figure(carry_events: pd.DataFrame, player_name: str) -> go.Figure:
    fig = _create_pitch_figure(f"{player_name} · {t('Carries')}")
    if carry_events.empty:
        return fig
    shown_legend = set()
    for _, carry in carry_events.iterrows():
        ...
        if obv > 0:
            color = "#3dd6a5"
            label = t("Positive OBV")
        elif obv < 0:
            color = "#ff6b6b"
            label = t("Negative OBV")
        else:
            color = "#9aa5bd"
            label = t("Zero OBV")
        hover_text = (
            f"<b>{player_name}</b><br>"
            f"{t('Minute')}: {carry.get('minute', 'N/A')}<br>"
            f"{t('OBV Total Net')}: {obv:.3f}"
        )
        fig.add_trace(go.Scatter(
            x=[x0, x1], y=[y0, y1],
            mode="lines+markers",
...
def _build_defensive_pitch_figure(defensive_events: pd.DataFrame, player_name: str) -> go.Figure:
    fig = _create_pitch_figure(f"{player_name} · {t('Defensive Actions')}")
    if defensive_events.empty:
        return fig
    for event_type, cfg in DEFENSIVE_TYPE_CONFIG.items():
        ...
        for _, event in subset.iterrows():
            ...
            if event_type == "Block":
                ...
                hover_info += f"{t('Body Part')}: {_event_value(event, ['clearance_body_part', 'clearance_body_part_name'], 'N/A')}<br>"
            elif event_type == "Ball Recovery":
                status = t("Failure") if event.get("ball_recovery_recovery_failure") == True else t("Success")
                hover_info += f"{t('Status')}: {status}<br>"
            elif event_type == "Duel":
                hover_info += f"{t('Duel Type')}: {_event_value(event, ['duel_type', 'duel_type_name'], 'N/A')}<br>"
                hover_info += f"{t('Duel Outcome')}: {_event_value(event, ['duel_outcome', 'duel_outcome_name'], 'N/A')}<br>"
            xs.append(x)
            ys.append(y)
...
def _build_heatmap_pitch_figure(location_events: pd.DataFrame, player_name: str) -> go.Figure:
    fig = _create_pitch_figure(f"{player_name} · {t('On-Ball Heatmap')}")
    if location_events.empty:
        return fig
    ...
    fig.add_trace(go.Histogram2d(
        ...
        zsmooth="best",
        colorbar=dict(title=t("Frequency"), len=0.74, thickness=14),
...
def _count_carry_obv(carry_events: pd.DataFrame) -> tuple[int, int]:
    if carry_events.empty:
        return 0, 0
    obv_series = _first_existing_series(carry_events, ["obv_total_net"])
    if obv_series is None:
        return 0, 0
    numeric = pd.to_numeric(obv_series, errors="coerce").fillna(0)
...
def _render_event_data_status(player_events: pd.DataFrame, raw_events_df: pd.DataFrame, player_name: str):
    with st.expander(t("Event Data Status"), expanded=False):
        st.write(t(
            "Loaded event rows: {count}",
            count=len(raw_events_df) if isinstance(raw_events_df, pd.DataFrame) else 0,
        ))
        st.write(t(
            "Filtered rows for player `{player_name}`: {count}",
        ...
        if isinstance(raw_events_df, pd.DataFrame) and not raw_events_df.empty:
            ...
            st.write(t("Available event columns:"), show_cols)
        else:
            st.info(t("No raw event rows were returned for this match."))


def _render_welcome():
    st.markdown(f"""
    <div style="text-align:center; padding: 60px 20px; color: #8b9bb4">
        <h2 style="font-size:2rem">📊 {t('Player Match Stats')}</h2>
        <p style="font-size:1rem">
            {t('Select a competition, season, match and player from the sidebar to begin.')}
        </p>
    </div>
    """, unsafe_allow_html=True)
