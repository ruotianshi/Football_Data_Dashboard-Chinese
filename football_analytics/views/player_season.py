...
def render():
    ...
    # 加载数据
    with st.spinner(t("Loading player data...")):
        df = load_player_season_stats(comp_id, season_id)

    if df.empty:
        st.warning(t("No player data available for this competition / season."))
        return
    ...
    if player_rows.empty:
        st.warning(t("Player not found in dataset."))
        return
...
def _render_sidebar_filters():
    with st.sidebar:
        ...
        st.divider()
        st.markdown(f"### 👤 {t('Player Selection')}")

        # 最低上场分钟数
        min_minutes = st.slider(
            t("Min. Minutes Played"),
        ...
        if not df.empty:
            ...
            if "team_name" in filtered.columns:
                ...
                team_list_all = [all_teams_value] + team_list

                prev_team = st.session_state.get("ps_team_name", all_teams_value)
                if prev_team not in team_list_all:
                    prev_team = all_teams_value
                if st.session_state.get("ps_team_select") not in team_list_all:
                    st.session_state["ps_team_select"] = prev_team
                ...
                selected_team = st.selectbox(
                    ...
                    key="ps_team_select",
                    format_func=lambda value: t("All Teams") if value == all_teams_value else value,
                )
                if selected_team != all_teams_value:
                    filtered = filtered[filtered["team_name"] == selected_team]
            else:
                selected_team = "__all_teams__"
            ...
            if filtered.empty:
                st.warning(t("No players match the current filters."))
                selected_player_id = None
            else:
                # 显示当前筛选结果数量
                st.caption(t("{count} players found", count=len(filtered)))
        ...
        st.session_state["ps_team_name"]         = st.session_state.get("ps_team_select", "__all_teams__")
...
def _render_profile_card(row):
    ...
    with col_info:
        ...
        <div style="padding: 4px 0">
            ...
            <p style="margin:6px 0; font-size:0.85rem">
                🎂 <b>{t('DOB')}:</b> {dob}{age_str}<br>
                📍 <b>{t('Position')}:</b> {pos_str}<br>
                📏 <b>{t('Height')}:</b> {height} cm &nbsp;·&nbsp; ⚖️ <b>{t('Weight')}:</b> {weight} kg
            </p>
        </div>
    ...
    with col_stats:
        ...
        m1.metric(t("Minutes"),  f"{minutes:,}")
        m2.metric(t("Apps"),     appearances)
        m3.metric(t("Avg Min"),  f"{avg_min:.0f}")
        m4.metric(t("90s"),      nineties)
...
def _render_radar_section(player_row, df):
    st.markdown(f"#### 🕸️ {t('Attribute Radar')}")

    min_min = st.session_state.get("ps_min_minutes", 500)

    # 指标组预设选择
    preset_options = ["Custom"] + list(RADAR_PRESET_GROUPS.keys())
    if st.session_state.get("radar_preset") not in preset_options:
        st.session_state["radar_preset"] = preset_options[0]
    preset = st.selectbox(
        t("Metric Preset"),
        preset_options,
        key="radar_preset",
        help=t("Choose a preset group or select custom metrics below"),
        format_func=t,
    )
    ...
    with col_norm:
        normalize = st.toggle(t("Percentile Rank"), value=True, key="radar_normalize",
                              help="ON: percentile vs league pool  OFF: min-max 0-100")
    with col_cmp_toggle:
        show_compare = st.toggle(t("Compare Player"), value=False, key="radar_compare_toggle")
    ...
    if show_compare:
        ...
        with cmp_col1:
            if "team_name" in pool.columns:
                cmp_team_list = ["__all_teams__"] + sorted(pool["team_name"].dropna().unique().tolist())
                if st.session_state.get("radar_compare_team") not in cmp_team_list:
                    st.session_state["radar_compare_team"] = "__all_teams__"
                cmp_team = st.selectbox(
                    t("Filter by Team"),
                    cmp_team_list,
                    key="radar_compare_team",
                    format_func=lambda value: t("All Teams") if value == "__all_teams__" else value,
                )
                if cmp_team != "__all_teams__":
        ...
        with cmp_col2:
            if cmp_options:
                cmp_name_sel = st.selectbox(
                    t("Select Compare Player"),
                    list(cmp_options.keys()),
                    key="radar_compare_player",
                )
            else:
                st.info(t("No players found."))
                cmp_name_sel = None
    ...
    if selected_metrics:
        ...
        st.info(t("Select at least one metric to display the radar chart."))
...
def _render_obv_section(player_row, df):
    st.markdown(f"#### 📈 {t('OBV Breakdown (p90)')}")
    ...
    # ── 额外指标卡片（Key Stats 快览）───────────────
    st.markdown(f"#### 📊 {t('Key Stats Overview')}")
...
def _render_bar_ranking_section(player_row, df):
    st.markdown(f"#### 🏆 {t('League Ranking')}")
    ...
    with ctrl1:
        ...
        if st.session_state.get("bar_metric_group") not in group_options:
            st.session_state["bar_metric_group"] = group_options[0]
        selected_group = st.selectbox(
            t("Metric Group"),
            group_options,
            key="bar_metric_group",
            format_func=t,
        )
        ...
        selected_metric_label = st.selectbox(
            t("Metric"), list(metric_label_map.keys()), key="bar_metric_select",
        )
    ...
    with ctrl3:
        top_n = st.slider(
            t("Show N Players"), min_value=5, max_value=50, value=25, step=5, key="bar_top_n",
        )

    with ctrl4:
        ascending = st.toggle(
            f"↑ {t('Ascending')}", value=False, key="bar_ascending",
            help=t("Controls display order only. Rankings always show top N by highest value."),
        )
    ...
    if "primary_position" in df.columns:
        ...
        selected_positions = st.multiselect(
            t("Position Filter  *(applied after ranking — target player always shown)*"),
            options=all_positions,
            default=[],
            key="bar_position_filter_multi",
            help=t("Leave empty to show all positions. Target player is always included regardless."),
        )
...
def _render_welcome():
    ...
    <div style="text-align:center; padding: 60px 20px; color: #8b9bb4">
        <h2 style="font-size:2rem">⚽ {t('Player Season')}</h2>
        <p style="font-size:1rem">
            {t('Select a competition, season, and player from the sidebar to begin.')}
        </p>
    </div>
    """, unsafe_allow_html=True)
