...
from utils.i18n import LANGUAGE_OPTIONS, ensure_language, language_label, t

DEFAULT_COMPETITION_NAME = "League Two"
...
_defaults = {
    ...
    "ps_player_id":        None,
    "ps_min_minutes":      500,
    "ps_competition_name": DEFAULT_COMPETITION_NAME,
    "ps_season_name":      DEFAULT_SEASON_NAME,
    "ts_competition_id":   None,
    "ts_season_id":        None,
    "ts_team_id":          None,
    "ts_competition_name": DEFAULT_COMPETITION_NAME,
    "ts_season_name":      DEFAULT_SEASON_NAME,
    "md_match_id":         None,
    "md_competition_name": DEFAULT_COMPETITION_NAME,
    "md_season_name":      DEFAULT_SEASON_NAME,
    "pms_match_id":        None,
    "pms_player_id":       None,
    "pms_competition_name": DEFAULT_COMPETITION_NAME,
    "pms_season_name":      DEFAULT_SEASON_NAME,
}
...
with st.sidebar:
    ...
    language_choices = list(LANGUAGE_OPTIONS.keys())
    current_language = st.session_state.get("ui_lang", "zh")
    selected_language_label = st.selectbox(
        t("Language"),
        language_choices,
        index=language_choices.index(language_label(current_language)),
        key="ui_language_select",
    )
    st.session_state["ui_lang"] = LANGUAGE_OPTIONS[selected_language_label]

    module_names = list(MODULE_ICONS.keys())
    if st.session_state.get("nav_radio") not in module_names:
        st.session_state["nav_radio"] = st.session_state.get("active_module", module_names[0])
    ...
    active_module = st.radio(
        ...
        format_func=lambda name: f"{MODULE_ICONS[name]}  {t(name)}",
    )
...
if active_module == "Player Season":
    ...
    try:
        ...
        st.info(f"🚧 {t('Team Season')} — {t('Coming soon.')}")
    ...
    try:
        ...
        st.info(f"🚧 {t('Match Dashboard')} — {t('Coming soon.')}")
    ...
    try:
        ...
        st.info(f"🚧 {t('Player Match Stats')} — {t('Coming soon.')}")
