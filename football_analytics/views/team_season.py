...
TEAM_METRIC_LABEL_OVERRIDES = {
    **{
        col: label
        for metrics in TEAM_METRIC_GROUPS.values()
        for col, label in metrics
    },
    "team_season_corner_xg_pg": "Corner xG pg",
    "team_season_completed_dribbles_conceded_pg": "Completed Dribbles Conceded pg",
}

TEAM_METRIC_DISPLAY_TRANSLATIONS = {
    "NP xG p90": "非点球 xG/90",
    "OP xG p90": "运动战 xG/90",
    "SP xG p90": "定位球 xG/90",
    "NP xG per Shot": "非点球每次射门 xG",
    "NP Shots p90": "非点球射门/90",
    "OP Shots p90": "运动战射门/90",
    "Deep Completions p90": "深入危险区域传球/90",
    "Passes into Box p90": "传入禁区/90",
    "Box Cross Ratio": "禁区传中占比",
    "Counter Att Shots p90": "反击射门/90",
    "High Press Shots p90": "高压射门/90",
    "xG Chain p90": "xG 链条/90",
    "NP xG Conceded p90": "非点球失 xG/90",
    "PPDA": "PPDA",
    "Defensive Distance": "防线高度",
    "Pressure Regains p90": "压迫夺回/90",
    "Counterpressure Regains p90": "反抢夺回/90",
    "F-Half Pressure %": "前场压迫占比",
    "Deep Comp Conceded p90": "被深入危险区域传球/90",
    "PAdj Tackles p90": "PAdj 抢断/90",
    "PAdj Interceptions p90": "PAdj 拦截/90",
    "Possession %": "控球率",
    "Pass Completion %": "传球成功率",
    "Directness": "直接性",
    "GK Pass Distance": "门将传球距离",
    "GK Long Pass %": "门将长传占比",
    "OP Passes p90": "运动战传球/90",
    "Pressures p90": "压迫/90",
    "Pace Towards Goal": "向球门推进速度",
    "OBV p90": "OBV/90",
    "OBV Pass p90": "传球 OBV/90",
    "OBV Shot p90": "射门 OBV/90",
    "OBV Def Action p90": "防守动作 OBV/90",
    "OBV Dribble/Carry p90": "盘带/推进 OBV/90",
    "OBV Pass": "传球 OBV",
    "OBV Shot": "射门 OBV",
    "OBV Def Action": "防守动作 OBV",
    "OBV Dribble/Carry": "盘带/推进 OBV",
    "Corner xG pg": "角球 xG/场",
    "Completed Dribbles Conceded pg": "被成功过人/场",
}

TEAM_METRIC_PHRASE_TRANSLATIONS = [
    ("Completed Dribbles Conceded", "被成功过人"),
    ("Deep Completions Conceded", "被深入危险区域传球"),
    ("Counterpressure Regains", "反抢夺回"),
    ("Pressure Regains", "压迫夺回"),
    ("Defensive Distance", "防线高度"),
    ("Pass Completion", "传球成功率"),
    ("Passes Into Box", "传入禁区"),
    ("Passes Inside Box", "传入禁区"),
    ("Counter Attacking Shots", "反击射门"),
    ("Counter Att Shots", "反击射门"),
    ("High Press Shots", "高压射门"),
    ("Deep Completions", "深入危险区域传球"),
    ("Deep Comp", "深入危险区域传球"),
    ("PAdj Interceptions", "PAdj 拦截"),
    ("PAdj Tackles", "PAdj 抢断"),
    ("Possession", "控球"),
    ("Pressures", "压迫"),
    ("Interceptions", "拦截"),
    ("Tackles", "抢断"),
...
DEFAULT_POINTS_TIMELINE_Y_MAX = 100
...
def render():
    ...
    # 加载数据
    with st.spinner(t("Loading team data...")):
        matches_df = load_matches(comp_id, season_id)
        team_stats = load_team_season_stats(comp_id, season_id)

    if matches_df.empty:
        st.warning(t("No match data available for this competition / season."))
        return
    ...
    try:
        ...
        st.error(t("Error rendering team header: {error}", error=e))
    ...
    if team_row is not None:
        try:
            ...
            st.error(t("Error rendering stats tabs: {error}", error=e))
        st.markdown('<hr style="border:1px solid #2a2d3e; margin:16px 0">', unsafe_allow_html=True)
    else:
        st.info(f"ℹ️ {t('Detailed team season stats require a paid StatsBomb subscription. Showing match-derived data below.')}")

    try:
        _render_points_timeline(matches_df, team_id)
    except Exception as e:
        st.error(t("Error rendering points timeline: {error}", error=e))
    ...
    if not team_stats.empty:
        try:
            ...
            st.error(t("Error rendering league ranking: {error}", error=e))
...
def _render_sidebar_filters():
    with st.sidebar:
        st.markdown(f"### 🔍 {t('Data Filters')}")

        comps_df = load_competitions()
        if comps_df.empty:
            st.error(t("No competitions available."))
            return
        ...
        comp_name = st.selectbox(t("Competition"), comp_names,
        ...
        season_name = st.selectbox(t("Season"), season_names,
        ...
        st.divider()
        st.markdown(f"### 🏟️ {t('Team Selection')}")
        ...
        with st.expander(f"🔧 {t('Debug: matches columns')}", expanded=False):
            if matches_df.empty:
                st.write(t("matches_df is **empty** — check credentials or competition/season."))
            else:
                st.write(
                    t("Rows: {rows} | Columns: {columns}", rows=len(matches_df), columns=list(matches_df.columns))
                )
                st.write(
                    t("Sample home_team value:"),
                    matches_df["home_team"].iloc[0] if "home_team" in matches_df.columns else t("column not found"),
                )
                st.write(t("team_options extracted:"), team_options)
        ...
        if team_options:
            ...
            st.caption(t("{count} teams in this competition", count=len(team_names)))
        else:
            st.warning(f"⚠️ {t('Could not load team list. Open the debug panel above to diagnose.')}")
...
def _render_team_header(team_row, team_id: int, matches_df: pd.DataFrame):
    ...
    with col_info:
        ...
        <div style="padding: 4px 0">
            ...
            <p style="margin:4px 0; font-size:0.85rem">
                👨‍💼 <b>{t('Manager')}:</b> {manager_name}<br>
                🏟️ <b>{t('Stadium')}:</b> {stadium_name}
            </p>
        </div>
    ...
    with col_record:
        if record:
            ...
            c1.metric(t("Played"),  record.get("played", 0))
            c2.metric(t("Won"),     record.get("won", 0))
            c3.metric(t("Drawn"),   record.get("drawn", 0))
            c4.metric(t("Lost"),    record.get("lost", 0))
            c5.metric(t("GF"),      record.get("gf", 0))
            c6.metric(t("GA"),      record.get("ga", 0))
            gd = record.get("gd", 0)
            c7.metric(t("GD"),      f"{'+' if gd >= 0 else ''}{gd}")

            st.markdown(
                f"<p style='font-size:1.15rem; color:{COLORS['primary']}; margin-top:6px'>"
                f"⭐ <b>{t('Points')}: {record.get('points', 0)}</b></p>",
                unsafe_allow_html=True,
            )
...
def _readable_team_metric_label(col: str) -> str:
    if col in TEAM_METRIC_LABEL_OVERRIDES:
        return TEAM_METRIC_LABEL_OVERRIDES[col]
    return col.replace("team_season_", "").replace("team_match_", "").replace("_", " ").title()


def _display_team_metric_label(label: str) -> str:
    if label in TEAM_METRIC_DISPLAY_TRANSLATIONS:
        return TEAM_METRIC_DISPLAY_TRANSLATIONS[label]

    translated = label
    translated = translated.replace("OBV Pass", "传球 OBV")
    translated = translated.replace("OBV Shot", "射门 OBV")
    translated = translated.replace("OBV Def Action", "防守动作 OBV")
    translated = translated.replace("OBV Dribble/Carry", "盘带/推进 OBV")
    translated = translated.replace("p90", "/90")
    translated = translated.replace("pg", "/场")
    for src, dst in TEAM_METRIC_PHRASE_TRANSLATIONS:
        translated = translated.replace(src, dst)
    return translated


def _points_timeline_axis_max(main_timeline: pd.DataFrame, compare_timelines: list[pd.DataFrame]) -> int:
    maxima = []
    if not main_timeline.empty and "cumulative_points" in main_timeline.columns:
        maxima.append(float(main_timeline["cumulative_points"].max()))

    for timeline in compare_timelines:
        if timeline.empty or "cumulative_points" not in timeline.columns:
            continue
        maxima.append(float(timeline["cumulative_points"].max()))

    actual_max = max(maxima) if maxima else 0.0
    return max(DEFAULT_POINTS_TIMELINE_Y_MAX, int(np.ceil((actual_max + 5) / 10.0) * 10))
...
def _render_stats_tabs(team_row, all_teams_df: pd.DataFrame):
    st.markdown(f"#### 📊 {t('Season Statistics')}")
    tab_names = list(TEAM_METRIC_GROUPS.keys())
    tabs = st.tabs([f"⚔️ {t(name)}" if name == "Attacking" else
                    f"🛡️ {t(name)}" if name == "Defending" else
                    f"🎯 {t(name)}" if name == "Possession" else
                    f"📈 {t(name)}" for name in tab_names])
...
def _render_team_stats_grid(team_row, all_df: pd.DataFrame, metrics: list):
    ...
    if not valid:
        st.info(t("No data available for this metric group."))
        return
        ...
        for j, (col, lbl) in enumerate(chunk):
            ...
            if isinstance(val, (int, float)) and not (isinstance(val, float) and np.isnan(val)):
                ...
                if not np.isnan(avg):
                    ...
                    cols[j].metric(_display_team_metric_label(lbl), fmt, delta=delta_fmt)
                else:
                    cols[j].metric(_display_team_metric_label(lbl), fmt)
            else:
                cols[j].metric(_display_team_metric_label(lbl), "N/A")
...
def _render_points_timeline(matches_df: pd.DataFrame, team_id: int):
    st.markdown(f"#### 📈 {t('Points Timeline')}")

    # ── 诊断展开器 ──────────────────────────────
    with st.expander(f"🔧 {t('Debug: Points Timeline')}", expanded=False):
        st.write(f"**{t('Passed team_id')}:** `{team_id}` (type: `{type(team_id).__name__}`)")
        st.write(f"**{t('matches_df rows')}:** {len(matches_df)}")
        if not matches_df.empty:
            cols = matches_df.columns.tolist()
            col_type = _detect_team_col_type(matches_df)
            st.write(f"**{t('Detected column type')}:** `{col_type}`")
    ...
    if main_timeline.empty:
        st.info(t("No match data to build points timeline."))
        return
    ...
    compare_sel = st.multiselect(
        f"➕ {t('Add teams to compare')}",
        options=compare_options,
        default=[],
        max_selections=4,
        key="ts_timeline_compare",
        help=t("Add up to 4 other teams from the same season to compare points progression"),
    )
    ...
    fig = go.Figure()
    compare_timelines: list[pd.DataFrame] = []
    ...
    for i, cmp_name in enumerate(compare_sel):
        ...
        if cmp_timeline.empty:
            continue
        compare_timelines.append(cmp_timeline)

        cmp_color = compare_colors[i % len(compare_colors)]
        cmp_hover = [
            f"<b>{cmp_name}</b><br>"
            f"{r['match_date'].strftime('%d %b %Y')}<br>"
            f"{'🏠 ' + t('Home') if r['venue']=='H' else '✈️ ' + t('Away')} vs {r['opponent']}<br>"
            f"{t('Score')}: {r['gf']}–{r['ga']} "
            f"{'✅' if r['result']=='W' else ('➖' if r['result']=='D' else '❌')}<br>"
            f"{t('Cumulative Points')}: <b>{t('{points} pts', points=r['cumulative_points'])}</b>"
    ...
    for _, r in main_timeline.iterrows():
        venue_str    = f"🏠 {t('Home')}" if r["venue"] == "H" else f"✈️ {t('Away')}"
        ...
        hover_texts.append(
            ...
            f"{t('Score')}: <b>{r['gf']}–{r['ga']}</b> {result_emoji}<br>"
            f"{t('Points this game')}: <b>{r['points_gained']}</b><br>"
            f"{t('Cumulative Points')}: <b>{t('{points} pts', points=r['cumulative_points'])}</b>"
            f"{mgr_str}{opp_mgr}"
        )
    ...
    axis_max = _points_timeline_axis_max(main_timeline, compare_timelines)

    # ── 参考线 ────────────────────────────────────
    max_possible_pairs = [
        (match_date, 3 * i)
        for i, match_date in enumerate(main_timeline["match_date"], start=1)
        if 3 * i <= axis_max
    ]
    avg_pace_pairs = [
        (match_date, 1.5 * i)
    ...
    fig.add_trace(go.Scatter(
        x=[item[0] for item in max_possible_pairs],
        y=[item[1] for item in max_possible_pairs],
        mode="lines", name=t("Max Possible"),
        line=dict(color=COLORS["grid"], width=1, dash="dot"),
        hoverinfo="skip",
    ))
    fig.add_trace(go.Scatter(
        x=[item[0] for item in avg_pace_pairs],
        y=[item[1] for item in avg_pace_pairs],
        mode="lines", name=t("Avg Pace (1.5 pts/g)"),
    ...
    fig.add_annotation(
        ...
        text=f"  <b>{t('{points} pts', points=last['cumulative_points'])}</b>",
    ...
    layout.update(dict(
        xaxis=dict(
            title=t("Match Date"), type="date",
            gridcolor=COLORS["grid"], linecolor=COLORS["grid"],
            tickformat="%d %b",
        ),
        yaxis=dict(
            title=t("Cumulative Points"),
            gridcolor=COLORS["grid"], linecolor=COLORS["grid"],
            rangemode="tozero",
            range=[0, axis_max],
        ),
    ...
    if not recent.empty:
        st.markdown(f"**{t('Recent Form (last 5 matches)')}**")
        cols = st.columns(5)
...
def _render_league_ranking(team_stats: pd.DataFrame, highlight_team_id: int):
    st.markdown(f"#### 🏆 {t('League Ranking Comparison')}")
...
def _get_all_numeric_metrics(df: pd.DataFrame) -> dict:
    ...
    for col in sorted(numeric_cols):
        label = _readable_team_metric_label(col)
        result[label] = col
    return result
...
def _render_bar_ranking_view(df: pd.DataFrame, highlight_id: int):
    ...
    if not all_metrics:
        st.info(t("No numeric metrics available."))
        return
    ...
    with ctrl1:
        metric_lbl = st.selectbox(
            ...
            format_func=_display_team_metric_label,
        )
    ...
    with ctrl3:
        ascending = st.toggle(f"↑ {t('Ascending')}", value=False, key="ts_bar_asc_v2")
...
def _build_team_bar(df, metric, metric_label, highlight_id, compare_ids, ascending):
    ...
    fig = go.Figure(go.Bar(
        ...
        hovertemplate="<b>%{y}</b><br>" + _display_team_metric_label(metric_label) + ": %{x:.3f}<extra></extra>",
    ))

    layout = {**PLOTLY_BASE_LAYOUT}
    layout.update(dict(
        title=dict(text=f"{t('League Ranking')} · <b>{_display_team_metric_label(metric_label)}</b>",
...
def _render_scatter_view(df: pd.DataFrame, highlight_id: int):
    ...
    if len(all_metrics) < 2:
        st.info(t("Not enough metrics for scatter plot."))
        return
    ...
    standardize = st.toggle(
        t("Standardise (z-score)"),
        value=True,
        key="ts_scatter_standardize",
        help=t("Normalise both axes to z-scores so metrics on different scales are comparable"),
    )
    ...
    if sub.empty:
        st.info(t("No data for selected metrics."))
        return

    raw_x_vals = sub[x_col].astype(float)
    raw_y_vals = sub[y_col].astype(float)
    x_vals = raw_x_vals.copy()
    y_vals = raw_y_vals.copy()

    x_axis_label = _display_team_metric_label(x_label)
    y_axis_label = _display_team_metric_label(y_label)
    ...
    if standardize:
        ...
        x_axis_label += f" ({t('Z-Score')})"
        y_axis_label += f" ({t('Z-Score')})"
    ...
    fig = go.Figure()
    fig.add_vline(
        x=0.0 if standardize else float(x_vals.mean()),
        line=dict(color=COLORS["grid"], width=1, dash="dot"),
    )
    fig.add_hline(
        y=0.0 if standardize else float(y_vals.mean()),
        line=dict(color=COLORS["grid"], width=1, dash="dot"),
    )

    customdata = np.column_stack([raw_x_vals, raw_y_vals, x_vals, y_vals])
    if standardize:
        hovertemplate = (
            "<b>%{text}</b><br>"
            + _display_team_metric_label(x_label) + f" ({t('Raw Value')}): " + "%{customdata[0]:.3f}<br>"
            + _display_team_metric_label(x_label) + f" ({t('Z-Score')}): " + "%{customdata[2]:.3f}<br>"
            + _display_team_metric_label(y_label) + f" ({t('Raw Value')}): " + "%{customdata[1]:.3f}<br>"
            + _display_team_metric_label(y_label) + f" ({t('Z-Score')}): " + "%{customdata[3]:.3f}<extra></extra>"
        )
    else:
        hovertemplate = (
            "<b>%{text}</b><br>"
            + _display_team_metric_label(x_label) + ": %{customdata[0]:.3f}<br>"
            + _display_team_metric_label(y_label) + ": %{customdata[1]:.3f}<extra></extra>"
        )
...
def _render_overview_radar(df: pd.DataFrame, highlight_id: int):
    ...
    if len(metric_keys) < 3:
        st.info(t("Insufficient metrics for radar chart."))
        return

    # ── 控制区 ────────────────────────────────────
    st.markdown(f"**{t('Radar Metrics Selection')}**")
    ...
    selected_labels = st.multiselect(
        t("Select metrics for radar (3–10)"),
        options=metric_keys,
        default=default_labels[:6],
        key="ts_radar_metrics",
        help=t("Choose 3 to 10 metrics to display on the league overview radar"),
        format_func=_display_team_metric_label,
    )

    if len(selected_labels) < 3:
        st.warning(t("Please select at least 3 metrics."))
        return
    if len(selected_labels) > 10:
        st.warning(t("Maximum 10 metrics. Only the first 10 will be used."))
    ...
    theta = [_display_team_metric_label(label) for label in labels_sel]
    theta = theta + [theta[0]]
    ...
    layout.update(dict(
        title=dict(
            text=t("League Overview · {count} Metrics (normalised 0–1)", count=len(selected_labels)),
            font=dict(size=13), x=0,
        ),
        showlegend=False,
...
def _render_welcome():
    ...
    <div style="text-align:center; padding: 60px 20px; color: #8b9bb4">
        <h2 style="font-size:2rem">🏟️ {t('Team Season')}</h2>
        <p style="font-size:1rem">
            {t('Select a competition, season, and team from the sidebar to begin.')}
        </p>
    </div>
    """, unsafe_allow_html=True)
