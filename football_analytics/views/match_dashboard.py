...
def render():
    ...
    # 加载数据
    with st.spinner(t("Loading match data...")):
    ...
    if matches_df.empty:
        st.warning(t("No match data available."))
        return

    # 找到对应的比赛行
    match_rows = matches_df[matches_df["match_id"] == match_id]
    if match_rows.empty:
        st.warning(t("Match not found."))
        return
...
def _render_sidebar_filters():
    with st.sidebar:
        ...
        st.divider()
        st.markdown(f"### ⚽ {t('Match Selection')}")
        ...
        if not matches_df.empty:
            ...
            if st.session_state.get("md_team_filter") not in ([all_teams_value] + sorted(all_teams)):
                st.session_state["md_team_filter"] = all_teams_value
            team_filter = st.selectbox(
                t("Filter by Team"),
                [all_teams_value] + sorted(all_teams),
                key="md_team_filter",
                format_func=lambda value: t("All Teams") if value == all_teams_value else value,
            )

            # 按球队过滤比赛
            if team_filter != all_teams_value:
            ...
            if match_options:
                ...
                selected_label = st.selectbox(t("Select Match"), match_labels,
...
def _build_match_options(matches_df: pd.DataFrame) -> dict:
    ...
    for _, row in matches_df.sort_values("match_date").iterrows():
        home_info = _extract_match_side_info(row, "home")
        away_info = _extract_match_side_info(row, "away")
        h_name = home_info["name"] or t("Home")
        a_name = away_info["name"] or t("Away")
...
def _render_team_stats(match_stats: pd.DataFrame, match_row):
    if match_stats.empty:
        st.info(t("No team stats available for this match."))
        return
    ...
    if home_row is None and away_row is None:
        st.info(t("No team data found in match stats."))
        return

    # 分组 tab
    stat_tabs = st.tabs([f"⚔️ {t('Attacking')}", f"🛡️ {t('Defending')}", f"🎯 {t('Possession')}"])
...
def _render_lineups(lineups_dict: dict, match_row, match_stats: pd.DataFrame, events_df: pd.DataFrame):
    if not lineups_dict:
        st.info(t("No lineup data available for this match."))
        return
...
def _render_welcome():
    ...
    <div style="text-align:center; padding: 60px 20px; color: #8b9bb4">
        <h2 style="font-size:2rem">⚽ {t('Match Dashboard')}</h2>
        <p style="font-size:1rem">
            {t('Select a competition, season, and match from the sidebar to begin.')}
        </p>
    </div>
    """, unsafe_allow_html=True)
