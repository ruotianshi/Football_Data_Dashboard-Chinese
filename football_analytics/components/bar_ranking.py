...
def _dedupe_player_rows(df: pd.DataFrame, metric: str) -> pd.DataFrame:
    """
    规范化球员赛季数据，确保同一个 player_id 只保留一行。

    有些赛季数据源会返回同一球员的多行记录（例如不同球队/不同拆分口径），
    若直接把 display_name 作为 y 轴分类，会在 Plotly 里叠成同一根柱子，
    进而出现柱长、标签、hover 数值互相错位的问题。

    这里优先保留：
      1. 出场分钟更多的行
      2. 目标指标非空且更大的行（仅作同分钟时的 tie-break）
      3. 非空字段更多的行
    """
    ...
    work["_minutes_sort"] = pd.to_numeric(
        work.get("player_season_minutes", 0),
        errors="coerce",
    ).fillna(-1)
    work["_metric_sort"] = pd.to_numeric(
        work.get(metric, 0),
        errors="coerce",
    ).fillna(float("-inf"))
    work["_completeness_sort"] = work.notna().sum(axis=1)

    work = work.sort_values(
        ["_minutes_sort", "_metric_sort", "_completeness_sort"],
        ascending=[False, False, False],
        kind="stable",
    )
    work = work.drop_duplicates(subset=["player_id"], keep="first")

    return work.drop(columns=["_minutes_sort", "_metric_sort", "_completeness_sort"], errors="ignore")


def _build_unique_display_labels(df: pd.DataFrame) -> pd.Series:
    """
    为图表 y 轴生成唯一标签，避免同名球员或残留重复记录在同一分类上重叠。
    """
    if df.empty or "display_name" not in df.columns:
        return pd.Series(dtype="object")

    name_counts = df["display_name"].astype(str).value_counts()
    labels = []
    seen = {}

    for _, row in df.iterrows():
        name = str(row.get("display_name", "")).strip() or "Unknown"
        team = str(row.get("team_name", "")).strip()
        position = str(row.get("primary_position", "")).strip()

        label = name
        if name_counts.get(name, 0) > 1:
            extras = [value for value in [team, position] if value]
            if extras:
                label = f"{name} ({' · '.join(extras[:2])})"
...
def build_bar_ranking(
    ...
    df = _dedupe_player_rows(df, metric)
    if df.empty:
        return go.Figure()
    ...
    top_df["bar_label"] = _build_unique_display_labels(top_df)
...
def build_obv_breakdown(player_row, all_df, min_minutes: int = 500) -> go.Figure:
    ...
    layout = {**PLOTLY_BASE_LAYOUT}
    layout.update(dict(
        title=dict(text=t("OBV Breakdown (p90)"), font=dict(size=13, color=COLORS["text"])),
        xaxis=dict(
